...
def test_postgres_worker_repository_records_transcription_events_without_transcript_text() -> None:
    constants = "\n".join(
        str(constant).lower()
        for constant in PostgresWorkerRepository.create_transcript.__code__.co_consts
    )
...
def test_postgres_worker_repository_records_failure_and_terminal_events() -> None:
    constants = "\n".join(
        str(constant).lower()
...
def test_postgres_worker_repository_records_terminal_completed_event() -> None:
    constants = "\n".join(
        str(constant).lower()
